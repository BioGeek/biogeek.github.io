...
(function () {
  ...
  // "PEPTIDE": each letter is a valid amino-acid one-letter code.
  ...
  // Build the peak model once (positions 0..1, heights 0..1, labels).
  var peaks = [];
  (function buildPeaks() {
    var rand = rng(20240517);
    // Choose seven evenly-ish spaced indices for the labeled fragment peaks.
    var labelSlots = [];
    for (var k = 0; k < RESIDUES.length; k++) {
      var base = (k + 0.5) / RESIDUES.length;
      labelSlots.push(Math.min(0.97, Math.max(0.03, base + (rand() - 0.5) * 0.05)));
    }
    for (var i = 0; i < TOTAL_PEAKS; i++) {
      var x = (i + 0.5) / TOTAL_PEAKS + (rand() - 0.5) * 0.012;
      peaks.push({ x: Math.min(0.99, Math.max(0.01, x)), h: 0.18 + rand() * 0.4, label: null });
    }
    // Snap each labeled fragment to the nearest unused peak and make it tall.
    for (var j = 0; j < labelSlots.length; j++) {
      var target = labelSlots[j];
      var best = -1, bestD = 2;
      for (var p = 0; p < peaks.length; p++) {
        if (peaks[p].label) continue;
        var d = Math.abs(peaks[p].x - target);
        if (d < bestD) { bestD = d; best = p; }
      }
      if (best >= 0) {
        peaks[best].label = RESIDUES[j];
        peaks[best].h = 0.74 + rand() * 0.2;
      }
    }
  })();
  ...
  var cursorX = 0.5;        // normalized 0..1 target
  var renderX = 0.5;        // eased value actually drawn
  var lastMove = 0;
  var autoPhase = 0;
  ...
  function draw() {
    var c = colors();
    var glowDist = 0.045; // normalized distance for "near cursor"
    ...
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(cx + 0.5, topPad - 14);
    ctx.lineTo(cx + 0.5, baseY);
    ctx.stroke();

    // Find labeled peak nearest the cursor (to reveal its residue).
    var nearestLabel = -1, nearestD = 2;
    for (var n = 0; n < peaks.length; n++) {
      if (!peaks[n].label) continue;
      var dd = Math.abs(peaks[n].x - renderX);
      if (dd < nearestD) { nearestD = dd; nearestLabel = n; }
    }

    for (var i = 0; i < peaks.length; i++) {
      var pk = peaks[i];
      var px = pk.x * W;
      var dist = Math.abs(pk.x - renderX);
      var prox = Math.max(0, 1 - dist / glowDist); // 0..1 closeness
      var grow = 1 + prox * 0.16;
      var ph = pk.h * usableH * grow;

      var isFrag = !!pk.label;
      var alpha = isFrag ? 0.55 : 0.32;
      alpha += prox * 0.4;

      if (prox > 0.02) {
        ctx.shadowColor = c.accent;
        ctx.shadowBlur = 10 * prox + (isFrag ? 4 : 0);
      } else {
        ctx.shadowBlur = 0;
      }
      ...
      ctx.lineWidth = isFrag ? 2.4 : 1.6;
      ctx.beginPath();
      ctx.moveTo(px, baseY);
      ctx.lineTo(px, baseY - ph);
      ctx.stroke();

      // Peak tip dot for fragment peaks.
      if (isFrag) {
        ctx.beginPath();
        ctx.arc(px, baseY - ph, 2.2 + prox * 1.6, 0, Math.PI * 2);
      ...
      // Reveal the residue letter for the labeled peak nearest the cursor.
      if (isFrag && i === nearestLabel && nearestD < glowDist * 1.4) {
        var reveal = Math.max(0, 1 - nearestD / (glowDist * 1.4));
        ...
        ctx.fillText(pk.label, px, baseY - ph - 8);
        ctx.shadowBlur = 0;
        ctx.globalAlpha = 1;
      }
    }
  ...
  function frame(now) {
    ...
    if (autoScan) {
      autoPhase += 0.0042;
      // Smooth back-and-forth across most of the spectrum width.
      cursorX = 0.5 + 0.46 * Math.sin(autoPhase);
    }
  ...
  // Redraw on theme toggle while idle (data-bs-theme / class changes).
  var mo = new MutationObserver(function () { draw(); });
  mo.observe(document.documentElement, { attributes: true,
    attributeFilter: ['class', 'data-bs-theme', 'style'] });

  window.addEventListener('resize', function () { resize(); draw(); }, { passive: true });

  resize();
  draw();              // sensible static first frame
  requestAnimationFrame(frame);
})();
</script>
