...
subtitle: "A full build log — calibration, a counterfeit controller, a flaky camera ribbon, power gremlins, and the fixes that actually worked"
description: >
  An end-to-end DonkeyCar build on a Jetson Nano, documented command-by-command:
  SSH and auth roadblocks, PCA9685 calibration, a counterfeit DualShock that the
  kernel refuses, an intermittent CSI camera ribbon, USB-PD over-voltage, DHCP
  surprises, and getting it all driving from a phone.
date: 2026-06-28
...
A training PC (RTX 4070) sits on the same network for the heavy lifting later.
